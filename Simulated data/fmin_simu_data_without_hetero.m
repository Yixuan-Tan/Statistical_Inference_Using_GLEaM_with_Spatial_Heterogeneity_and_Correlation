function loss = fmin_simu_data_without_hetero(x, ytrue, ini, loss_type, dt, endday, startday)

% function to compute the loss of the given parameters x
% this function is for the models that **do not** allow the parameter heterogeneity
...
lambdavec = x(2*n+1) * ones(1, n); % before

gamma  = ini.gamma; % value
delta  = ini.delta; % value

% W_reg  = ones(n) * mu * traVal;
% 
...
ytrue = ytrue(startday:endday, 1:n);
%--------------------------------------------------------

if strcmp(loss_type, 'pois')
    y    = yC(startday: endday, :);
    loss = -sum(ytrue(:) .* log(y(:)) - y(:));% + lambdavec * Lap_reg * lambdavec';

elseif strcmp(loss_type, 'norm')
    y    = max([yC(startday: endday, :), yCR(startday: endday, :)], 1);
