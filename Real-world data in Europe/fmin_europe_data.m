function loss = fmin_europe_data(x, newly_conf_true, ini, loss_type, dt, mu, beta, startday,endday,sigma)
...
delta               = ini.delta;
gamma               = ini.gamma;
lambda_before       = x((2*n_country+1):(3*n_country));
...
adjac_mat = mu1 * ini.commu_mat_within_clust + mu0 * ini.commu_mat_between_clust;
GL_mat = diag(sum(adjac_mat)) - adjac_mat;
...
if strcmp(loss_type, 'pois')
    ...
    loss = -sum(newly_conf_true(:) .* log(newly_conf_est(:)) - newly_conf_est(:))+lambda_later*GL_mat *lambda_later' + lambda_before * GL_mat * lambda_before'...
        + sigma*norm(lambda_later,2)^2 + sigma*norm(lambda_before,2)^2;
    %loss = -sum(newly_conf_true(:) .* log(newly_conf_est(:)) - newly_conf_est(:))+ 1e5*lambda_later * GL_mat * lambda_later'...
    %     + sigma*norm(lambda_later,2)^2 + sigma*norm(lambda_before,2)^2;
    
elseif strcmp(loss_type, 'norm') % to be revised
    y    = [yC1, yCR1];
    %    loss = norm(y(:) - ytrue(:), 2)^2 / temp;
    loss = sum(diag(((y - ytrue1)./(max(ytrue1,1)).^(1/2)) * ((y - ytrue1)./(max(ytrue1,1)).^(1/2))'))+lambda_later * GL_mat * lambda_later' ;
end
