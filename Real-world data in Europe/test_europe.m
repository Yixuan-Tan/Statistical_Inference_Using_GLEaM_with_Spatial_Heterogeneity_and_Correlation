clear all;
load("data_averaged.mat"); % need to run file data_preprocess.m and data_overview.m before running this file
...
% group_cell{1} = [1;2;3]; % first type of group assignment
% group_cell{2} = [4;5;6];
% group_cell{3} = [7;8];
% group_cell{4} = [9;10;11];

group_cell{1} = [2;3] % second type of group assignment
group_cell{2} = [1;4;5]
...
change_day = 50; % the day that lambda_k changes
...
for i = 1:length(muvec)
    ...
    y_reg = real_world_data_generate_determ(params1, ini, dt, trainDays_begin1, validDays_end);
    ...
    y_reg = real_world_data_generate_determ(params1, ini, dt, trainDays_begin2, testDays_end);
...
tic;
for i = 1: numsamp
    [traject_temp_newly, ~, traject_temp_accu] = real_world_data_generate_random(params_temp1, ini, trainDays_begin1, totDays);
...
proposal_dist  = 1e-8 * ones(1, length(params1)); % proposal distribution
proposal_dist(1:(1+2*n_country)) = zeros(1, 1+2*n_country); % only estimate posterior distribution for lambda_k
iterations    = 5e5; % total number of sampling steps

numtrasamp    = 1e2; % total number of sampling random trajectories
...
for i = 1: iterations
    ...
    if mod(i, step) == 0 % generate random trajectories based on sampled parameters
...
%       
        A = real_world_data_generate_random(params_temp1, newly_confirmed_data_ave(1:ini.N_days,:), ini, 'pois', dt, mu, beta, trainDays_begin2, trainDays_end2, sigma) - ...
            real_world_data_generate_random(params_temp2, newly_confirmed_data_ave(1:ini.N_days,:), ini, 'pois', dt, mu, beta, trainDays_begin2, trainDays_end2, sigma);
        Avec(i) = A;    
        A = exp(A);
    else
        A = 0;
